...
class InsuranceDetailController:
    ...
    async def create_insurance_detail_journey(
        ...
        Args:
            payload: Customer insurance-detail form data for a new journey.

        Returns:
            InsuranceDetailCreateResponse: Created user, transaction, and
                insurance-detail identifiers with the current status.

        Raises:
            HTTPException: If required customer identifiers are missing or the
                journey cannot be created.
        """
        try:
            logging.info(
            ...
            if not normalized_mobile_number:
                logging.warning(
                    "Empty mobile number provided for insurance-detail journey creation"
                )
                raise HTTPException(
    ...
    async def update_insurance_detail(
        ...
        Args:
            transaction_id: Business transaction identifier linked to the
                insurance-detail document.
            payload: Partial insurance-detail updates from the customer journey.

        Returns:
            InsuranceDetailUpdateResponse: Updated transaction and
                insurance-detail state.

        Raises:
            HTTPException: If the transaction identifier is invalid or the
                linked insurance detail cannot be updated.
        """
        try:
            logging.info(
            ...
            if not normalized_transaction_id:
                logging.warning(
                    "Empty transaction_id provided for insurance-detail update"
                )
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
            ...
            if transaction is None:
                logging.warning("Transaction not found for id %s", normalized_transaction_id)
                raise HTTPException(
            ...
            if insurance_detail is None:
                logging.warning(
                    "Insurance detail not found for transaction %s",
                    normalized_transaction_id,
                )
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
    ...
    async def get_latest_incomplete_journey(
        ...
        Args:
            mobile_number: Customer mobile number used to find the latest
                incomplete journey.

        Returns:
            LatestIncompleteInsuranceDetailResponse: Latest incomplete journey
                details for the customer.

        Raises:
            HTTPException: If the mobile number is invalid or no matching
                incomplete journey can be found.
        """
        try:
            logging.info(
            ...
            if not normalized_mobile_number:
                logging.warning(
                    "Empty mobile number provided for latest incomplete journey lookup"
                )
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail="Mobile number is required.",
    ...
    async def _generate_quotes_for_transaction(
        ...
        try:
            logging.info(
                "Executing InsuranceDetailController._generate_quotes_for_transaction function"
            )
            if transaction.current_status != TransactionStatus.FORM_SUBMITTED:
                logging.info(
                    "Skipping quote generation for transaction %s because status is %s",
                    transaction.transaction_id,
                    transaction.current_status.value,
                )
                return
            ...
            logging.info(
                "Quote generation completed successfully for transaction %s",
                transaction.transaction_id,
            )
        except HTTPException as httperror:
            logging.error(
                "Error in InsuranceDetailController._generate_quotes_for_transaction function: %s",
                httperror,
            )
            raise httperror
        except Exception as error:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Failed to generate quotes for the transaction.",
            )
