...
if not JWT_SECRET:
    raise RuntimeError("JWT_SECRET_KEY is required in provider_backend/.env")


def signJWT(user_role: str, id: str, expiry_duration: int | None = None) -> str:
    """
    Create a signed JWT access token for the given provider-side user.

    Args:
        user_role: Role string for the provider-side user
...
def sign_jwt(user_role: str, user_id: str, expiry_duration: int | None = None) -> str:
    """Create a signed JWT access token for the given provider-admin user."""
