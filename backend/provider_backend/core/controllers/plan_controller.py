...
class PlanController:
    ...
    async def create_plan(self, payload: PlanCreateRequest) -> PlanResponse:
        ...
        try:
            logging.info("Executing PlanController.create_plan")
            ...
            logging.error("Error in PlanController.create_plan: %s", error)
            raise HTTPException(
    ...
    async def update_plan(self, plan_code: str, payload: PlanUpdateRequest) -> PlanResponse:
        ...
        try:
            logging.info("Executing PlanController.update_plan")
            ...
            if (
                ...
                updates["available_add_ons"] = [
                    EmbeddedAddOn(**PlanAddOnRequest(**item).model_dump())
                    if isinstance(item, dict)
                    else EmbeddedAddOn(**item.model_dump())
            ...
            logging.error("Error in PlanController.update_plan: %s", error)
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def get_plan(self, plan_code: str) -> PlanResponse:
        ...
        try:
            logging.info("Executing PlanController.get_plan")
            plan = await self.plan_crud.get_by_plan_code(plan_code)
            if plan is None:
                logging.warning("Plan not found for code %s", plan_code)
                raise HTTPException(
            ...
            logging.error("Error in PlanController.get_plan: %s", error)
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    async def list_plans(self) -> PlanListResponse:
        ...
        try:
            logging.info("Executing PlanController.list_plans")
            ...
            logging.error("Error in PlanController.list_plans: %s", error)
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
